...
class QMIX_MLP:
    def __init__(self, args):
        ...
        self.eval_rnn_policy = MLP(input_shape, args.n_actions, args)  # 每个agent选动作的网络
        self.target_rnn_policy = MLP(input_shape, args.n_actions, args)
        self.eval_vdn_net = QMixNet(args)  # 把agentsQ值加起来的网络
        self.target_vdn_net = QMixNet(args)
        ...
        self.eval_parameters = list(self.eval_vdn_net.parameters()) + list(self.eval_rnn_policy.parameters())
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        q_total_eval = self.eval_vdn_net(q_evals, s)
        q_total_target = self.target_vdn_net(q_targets, s)
    ...
    def get_q_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            ...
            # print(inputs.shape)
            # q_eval, self.eval_hidden = self.eval_rnn_policy(inputs,
            #                                          self.eval_hidden)
            # q_target, self.target_hidden = self.target_rnn_policy(inputs_next, self.target_hidden)

            # print(inputs.size())
            # print(self.obs_shape, self.n_agents * (self.n_actions + 2))
            q_eval = self.eval_rnn_policy(inputs)
            q_target = self.target_rnn_policy(inputs_next)
